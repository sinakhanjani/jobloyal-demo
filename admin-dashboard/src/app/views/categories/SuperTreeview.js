...
class SuperTreeview extends Component {
    ...
    componentWillReceiveProps(nextProps) {
        if (!isEqual(nextProps.data, this.props.data)) {
            this.setState({ data: cloneDeep(nextProps.data.filter((e) => !e.deleted)) });
        }
    }
    ...
    handleAdd(node) {
        const {onAddCb} = this.props
        onAddCb(node)
    }

    handleEdit(node) {
        const {onEditCb} = this.props
        onEditCb(node)
    }

    handleJobs(node) {
        const {onJobsCb} = this.props
        onJobsCb(node)
    }
    ...
    printDeleteButton(node) {
        const { isDeletable, depth, deleteElement,addElement,jobsElement } = this.props;
    ...
    printExpandButton(node) {
        ...
        if (isExpandable(node, depth) && node.children) {
            return (
                <div
...
SuperTreeview.propTypes = {
    ...
    onEditCb: PropTypes.func,
    onJobsCb: PropTypes.func,
    onAddCb: PropTypes.func,
