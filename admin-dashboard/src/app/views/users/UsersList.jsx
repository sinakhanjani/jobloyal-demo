...
const fetchData = async (pagination) => {
    const response = await axios.post("/users",pagination);
...
const UsersList = () => {
    ...
    useEffect(() => {
        if (search.hasOwnProperty("text") && search.text.length > 1) {
            ...
            axios.post("/users/search",{page: page, limit: rowsPerPage,s: search.text}).then(result => {
                if (result.data.data) {
                        ...
                        .map((subscriber, index) => (
                            ...
                            <TableRow key={index} style={{display: 'content'}}>
                                <TableCell className="px-0 capitalize" align="left">
                                    <Link to={`/dashboard/user/${subscriber.id}`} style={{display: "content"}}>
                                    {subscriber.name}
                                    </Link>
                                </TableCell>
                                <TableCell className="px-0 capitalize" align="left">
