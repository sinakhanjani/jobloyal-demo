...
class JobberOrderInRequestArrivedViewController: JobberViewController, PopupConfiguration {
    ...
    private func updateUI() {
        ...
        let buttonTitle = isNumeric ?  "I Started The Service".localized():"I Started The Service(s)".localized()
        ...
        serviceButton.backgroundColor = .heavyBlue
    ...
    private func startServiceRequest(sender: UIButton) {
        let network = RestfulAPI<EMPTYMODEL,Generic<EMPTYMODEL>>.init(path: "/jobber/request/start")
            .with(auth: .jobber)
            .with(method: .POST)
    ...
    @IBAction func startServiceButtonTapped(_ sender: Any) {
        startServiceRequest(sender: sender as! UIButton)
    }
...
extension JobberOrderInRequestArrivedViewController: UITableViewDelegate {
